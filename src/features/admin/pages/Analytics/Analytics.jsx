    ...
    <div className="min-h-screen bg-slate-950 p-6 pb-12 space-y-6">
          ...
          <div className="flex gap-1 p-1 bg-slate-900 border border-slate-700 rounded-lg">
            {["Week", "Month", "Quarter", "Year"].map((p) => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={`px-3 py-1.5 rounded-md text-[11px] font-bold transition-all ${period === p ? "bg-indigo-600 text-white shadow-sm" : "text-slate-400 hover:text-white"}`}
              >
                {p}
              </button>
            ))}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {KPI_CARDS.map((k) => (
          <div
            key={k.label}
            className="bg-slate-900 rounded-2xl p-5 border border-slate-800 flex items-center gap-4 hover:border-slate-700 transition-colors"
          >
            <div className={`${k.color} p-3 rounded-xl flex-shrink-0 opacity-90`}>
              <span
            ...
            <div>
              ...
              <span
                className={`inline-flex items-center gap-0.5 text-[11px] font-bold ${k.up ? "text-emerald-400" : "text-red-400"}`}
              >
          ...
          <div className="space-y-2 mt-2">
            {incidentTypes.map(({ name, value, color }) => (
              <div key={name} className="flex items-center gap-2.5">
                ...
                />
                <span className="text-[12px] text-slate-400 flex-1">{name}</span>
                ...
                <span className="text-[12px] font-bold text-slate-200 w-8 text-right">
                  {value}%
                </span>
              </div>
            ))}
            ...
            <thead>
              <tr className="bg-slate-800/50 border-b border-slate-800">
                {[
                  ...
                  <th
                    key={h}
                    className="text-left px-4 py-3 text-[10px] text-slate-400 font-bold uppercase tracking-wider"
                  >
                    {h}
                  </th>
                ))}
              ...
              {districtHeatmap.map((d, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      <div className="w-28 h-3 bg-slate-800 rounded-full overflow-hidden">
                        <div
                          className="h-full rounded-full"
                          style={{
                            width: `${heat}%`,
                            background: `hsl(${210 - heat * 1.4}, 70%, 50%)`,
                          }}
                        />
                      </div>
                    </td>
